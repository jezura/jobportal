...
public class CVExtractor {
    ...
    public void processCvAndSetTextContentToExtractedTextVariable(MultipartFile[] files) {
        ...
        // - Heroku deployed version --> Path fileNameAndPath = Paths.get(new ClassPathResource("filename").getPath());
        try {
        ...
        // - Heroku deployed version --> File savedFile = new File(String.valueOf(fileNameAndPath));
    ...
    /**
     * Method extracts whole text from CV file
     *
     * @param file     - File (attached PDF/DOCX CV file)
     * @param fileName - fileName of attached CV file
     * @return extractedText String variable
     */
    ...
    /**
     * Method finds and extracts email from the attached CV file
     *
     * @return String with extracted information (email)
     */
    ...
    /**
     * Method finds and extracts mobile number from the attached CV file
     *
     * @return String with extracted information (mobile number)
     */
    ...
    /**
     * Method finds and extracts all gained academic titles from the attached CV file
     *
     * @param titles - Collection of all most common academic titles from database
     * @return List<Title> titlesList with extracted information (list of all gained titles)
     */
    public List<Title> extractTitle(Collection<Title> titles) {
        ...
        for (Title title : titles) {
            ...
            if (matcher.find()) {
                ...
                // if it is unique - add to titles list
                if (unique) {
    ...
    /**
     * Method finds and extracts first name of the owner of CV from the attached CV file
     *
     * @param czechNames      - Collection of all most common czech names from database
     * @param sizeOfStartArea - the number of characters in the text search area on the first search
     * @return CzechName czechName extracted information (matched firstname from czech names table)
     */
    public CzechName extractFirstName(Collection<CzechName> czechNames, int sizeOfStartArea) {
        ...
        // if the previous method failed - try to find first name in whole CV text
        for (CzechName czechName : czechNames) {
            String regexName = "\\s?(" + czechName.getName() + "\\s|" + czechName.getName().toUpperCase() + "\\s)";
            Pattern pattern = Pattern.compile(regexName);
            Matcher matcher = pattern.matcher(extractedText);
    ...
    /**
     * Method finds and extracts last name of the owner of CV from the attached CV file
     *
     * @param firstName - extracted firstName from previous method
     * @return String lastName extracted information (last name extracted with the help of the extracted first name)
     */
    ...
    /**
     * Method finds and extracts birth date or birth year (if the full date is not present)
     * of the owner of CV from the attached CV file. If there is no birth date or year mentioned,
     * this method will try to estimate the birth year from the context of all other years
     * mentioned in the text of the CV.
     *
     * @return LocalDate object extracted information (Birth date / birth year)
     */
    ...
    /**
     * This method finds and extracts education information (education level and education general field)
     * of the owner of CV from the attached CV file. Method uses text area indexes and sections variable
     * size improvement methods
     *
     * @param titles - Collection of all most common academic titles from database
     * @return MaxEducation object with extracted education information (education level and general study field)
     */
    public MaxEducation extractMaxEducationAndGeneralEduField(Collection<Title> titles) {
        ...
        // try to obtain extracted information from the gained academic titles
        if (!titles.isEmpty()) {
            for (Title title : titles) {
            ...
            if (maxEducation.getEduGeneralField() != null) {
                ...
                // if the general edu field could not be detected during the previous attempt
                // try to find and extract this information from text using area indexes and text sections improvement methods
            ...
            // if there were no titles
            // try to find and extract all information from text using area indexes and text sections improvement methods
            ...
            switch (maxEduLvl.getEduLevel().getName()) {
                ...
                case "Vyssi_odborne":
                case "Stredoskolske_s_maturitou":
