...
@WebAppConfiguration
public class MaxEduLvlTest {
    @Autowired
    EduLevelRepository eduLevelRepository;

    /**
     * Test of MaxEduLvl text area indexes/areas finding accuracy on sample CV text content
     * Tests if found and set VS education text area indexes (start and end) was set properly as expected
     */
    @Test
    public void testFindAreaIndexesForVSLevelMethodAccuracy() {
        ...
        assertEquals(0, maxEduLvl.getStartPosIndex());
        assertEquals(0, maxEduLvl.getEndPosIndex());

        maxEduLvl.findAreaIndexesForVSLevel(returnSampleTestingExtractedTextFromCV(), 70, eduLog);

        assertEquals(208, maxEduLvl.getStartPosIndex());
        assertEquals(440, maxEduLvl.getEndPosIndex());
    ...
    /**
     * Test of MaxEduLvl object max education level finding accuracy on sample CV text content
     * Tests if found and set education level was found as expected
     */
    @Test
    public void testFindMaxEduLvlMethodFindingAccuracy() {
        MaxEduLvl maxEduLvl = new MaxEduLvl(eduLevelRepository);

        EduLog eduLog = new EduLog();

        assertTrue(eduLog.getLogText().isEmpty());
        assertNull(maxEduLvl.getEduLevel());

        maxEduLvl.findMaxEduLvl(returnSampleTestingExtractedTextFromCV(), 0, 70, eduLog);

        assertEquals(maxEduLvl.getEduLevel().getName(), "Vysokoskolske_magisterske");
        assertFalse(eduLog.getLogText().isEmpty());
